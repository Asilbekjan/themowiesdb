...
export default function Tvshow() {
    ...
    useEffect(()=>{
        axios({
            method:'GET',
            url:`https://api.themoviedb.org/3/tv/${sorting}?language=en-US&page=1`,
            headers: {
                Authorization:
                       ...
                       return (
                           <div key={index} className="col-span-1 my-4 ">
                             <div className="card drop-shadow-lg ">
                                 ...
                                 <p className="text-center font-semibold mt-2 mx-1 "><Link to={`/movieinfo/${item.id}`} className='hover:text-blue-500'>{item.original_name}</Link></p>
                                 <p className="text-center mt-1">{item.first_air_date}</p>
                             </div>
                           </div>
                       );
                     })
